  ...
  return (
    <>
      {Array.from({ length: count }).map((_, index) => (
        <div
          key={index}
          className="box-border p-1 flex-[0_0_50%] animate-pulse"
        >
          <div className="relative pb-[100%]">
            <div className="flex flex-col items-center justify-center h-full left-0 absolute top-0 w-full bg-gray-300 rounded-md">
              <svg
              ...
              <div className="absolute bottom-0 h-3 bg-gray-200 rounded-full w-[90%] mb-3"></div>
            </div>
          </div>
        </div>
      ))}
