...
function usePlaces(props) {
  ...
  useEffect(() => {
    const newPlacesToRender =
      placesCtx.filteredPlaces.length > 0
        ? placesCtx.filteredPlaces
        : placesCtx.places;
    setPlacesToRender(newPlacesToRender);
  }, [placesCtx.filteredPlaces, placesCtx.places]);

  useEffect(() => {
    if (placesCtx.places.length > 0) {
      if (map && props?.markerToRemove) {
        // const markerIdToRemove = placesToRender.findIndex(
        //   (element) => element._id === props.markerToRemove
        // );
        // if (markerIdToRemove !== -1) {
        //   placesToRender.splice(markerIdToRemove, 1);
        // }

        // const pla = placesToRender.filter(
        //   (element) => element._id !== props.markerToRemove
        // );
        // console.log(pla.length);
